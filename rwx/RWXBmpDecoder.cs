...
namespace RWXLoader
{
    ...
    public class RWXBmpDecoder : MonoBehaviour
    {
        ...
        public IEnumerator DecodeBmpTextureAsync(byte[] bmpData, string fileName, Action<Texture2D> onComplete, int rowsPerFrame = 64)
        {
            ...
            if (width <= 0 || height == 0 || dataOffset < 0 || dataOffset >= bmpData.Length)
            {
                onComplete?.Invoke(null);
                yield break;
            }
            ...
            if (bitsPerPixel == 1)
            {
                ...
                for (int y = 0; y < absHeight; y++)
                {
                    ...
                    {
                        int byteIndex = rowStart + (x / 8);
                        if (byteIndex < 0 || byteIndex >= bmpData.Length)
                        {
                            Object.DestroyImmediate(texture);
                            onComplete?.Invoke(null);
                            yield break;
            ...
            else
            {
                ...
                for (int y = 0; y < absHeight; y++)
                {
                    ...
                    {
                        int pixelIndex = dataOffset + (sourceY * rowSize) + (x * bytesPerPixel);
                        if (pixelIndex < 0 || pixelIndex + bytesPerPixel > bmpData.Length)
                        {
                            Object.DestroyImmediate(texture);
                            onComplete?.Invoke(null);
                            yield break;
