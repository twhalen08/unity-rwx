...
namespace RWXLoader
{
    public class RWXParser
    {
        ...
        private readonly Regex nonCommentRegex = new Regex(@"^(.*)#(?!\!)", DefaultRegexOptions);
        private readonly Regex attributeRegex = new Regex(@"\b(mask|normal|specular)\s+([A-Za-z0-9_\-\/:.]+)", DefaultRegexOptions);
        ...
        private bool ProcessTexture(string line, RWXParseContext context)
        {
            ...
            var attrMatches = attributeRegex.Matches(rest);
