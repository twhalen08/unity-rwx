...
namespace RWXLoader
{
    public class RWXParser
    {
        ...
        public RWXParser(RWXMeshBuilder meshBuilder, bool enableDebugLogs = false)
        {
            this.meshBuilder = meshBuilder;
            this.enableDebugLogs = enableDebugLogs;
            this.prototypeParser = new RWXPrototypeParser(this, meshBuilder, enableDebugLogs);
            ...
            if (match.Groups[7].Success)
            {
                ...
                if (uvMatches.Count >= 2)
                {
                    ...
                    if (prototypeParser.IsInPrototype && NeedsUVCorrection(context))
                    {
                        ...
                        LogDebug($"🎨 UV CORRECTION: Applied additional UV flip for prototype texture orientation");
                    }
                }
            }
        ...
        private bool ProcessClumpBegin(string line, RWXParseContext context)
        {
            ...
            LogDebug($"🎯 CLUMP BEGIN - Depth: {depth}");
            LogDebug($"   📦 Created: '{clumpName}' - clump-local matrix will be derived from parent on clumpend");
            LogDebug($"   ═══════════════════════════════════════");
        ...
        internal Matrix4x4 ConvertRWXMatrixToUnity(Matrix4x4 rwxMatrix, RWXParseContext context)
        {
            ...
            LogDebug($"🔄 MATRIX CONVERSION | {hierarchyPath}");
            LogDebug($"   RWX Translation: ({rwxMatrix.m03:F6}, {rwxMatrix.m13:F6}, {rwxMatrix.m23:F6})");
            LogDebug($"   Unity Translation: ({unityMatrix.m03:F6}, {unityMatrix.m13:F6}, {unityMatrix.m23:F6})");
            LogDebug($"   Stack Depth: {context.objectStack.Count}");
        ...
        internal void ApplyTransformToObject(Matrix4x4 rwxMatrix, GameObject target, RWXParseContext context)
        {
            ...
            if (TryDecomposeMatrix(unityMatrix, out position, out rotation, out scale))
            {
                ...
                LogDebug($"   🔄 Applied accumulated transform:");
                LogDebug($"   📍 Local Position: {position:F6}");
                LogDebug($"   🔄 Local Rotation: {rotation}");
                LogDebug($"   📏 Local Scale: {scale:F6}");
            }
            else
            {
                ...
                LogDebug($"   📍 Applied fallback position: {fallbackPosition:F6}");
            }
        }
        ...
        private bool ProcessClumpEnd(string line, RWXParseContext context)
        {
            ...
            LogDebug($"🏁 CLUMP END - Depth: {depth}");
            LogDebug($"   📦 Ending: '{currentName}'");
            ...
            if (context.currentObject != null)
            {
                ...
                {
                    LogDebug("   ⏭️ Skipping clump transform (prototype instances already baked)");
                }
                else
                {
            ...
            if (context.objectStack.Count > 0)
            {
                ...
                LogDebug($"   ⬆️ Returning to parent: '{parentName}'");
            }
        ...
        private bool ProcessTransformBegin(string line, RWXParseContext context)
        {
            ...
            LogDebug($"🔄 TRANSFORM BEGIN - Stack depth: {context.transformStack.Count}");
            LogDebug($"   📋 Pushed current transform to stack");
            LogDebug($"   🎭 Current transform matrix:");
            LogDebug($"      [{context.currentTransform.m00:F3}, {context.currentTransform.m01:F3}, {context.currentTransform.m02:F3}, {context.currentTransform.m03:F3}]");
            LogDebug($"      [{context.currentTransform.m10:F3}, {context.currentTransform.m11:F3}, {context.currentTransform.m12:F3}, {context.currentTransform.m13:F3}]");
            LogDebug($"      [{context.currentTransform.m20:F3}, {context.currentTransform.m21:F3}, {context.currentTransform.m22:F3}, {context.currentTransform.m23:F3}]");
            LogDebug($"      [{context.currentTransform.m30:F3}, {context.currentTransform.m31:F3}, {context.currentTransform.m32:F3}, {context.currentTransform.m33:F3}]");
        ...
        private bool ProcessTranslate(string line, RWXParseContext context)
        {
            ...
            LogDebug($"🔄 TRANSLATE: ({translationVector.x:F6}, {translationVector.y:F6}, {translationVector.z:F6}) | Old pos: {oldPos:F6} → New pos: {newPos:F6}");
        ...
        private bool ProcessRotate(string line, RWXParseContext context)
        {
            ...
            if (isRootLevelRotation)
            {
                ...
                LogDebug($"🎯 ROOT-LEVEL ORIENTATION: Axis({axisX:F1}, {axisY:F1}, {axisZ:F1}) Angle({angleDegrees:F1}°) - keeping original");
            }
            else
            {
                ...
                if (Mathf.Abs(axisX) > 0.9f) // Rotation around X axis
                {
                    ...
                    LogDebug($"🔄 Internal X-axis rotation: negating angle {angleDegrees}° → {unityAngle}°");
                }
                else if (Mathf.Abs(axisY) > 0.9f) // Rotation around Y axis
                {
                    unityAngle = -angleDegrees;
                    LogDebug($"🔄 Internal Y-axis rotation: negating angle {angleDegrees}° → {unityAngle}°");
                }
                else if (Mathf.Abs(axisZ) > 0.9f) // Rotation around Z axis
                {
                    unityAngle = -angleDegrees;
                    LogDebug($"🔄 Internal Z-axis rotation: negating angle {angleDegrees}° → {unityAngle}°");
                }
            }
            ...
            LogDebug($"🔄 ROTATE RESULT: Axis({unityAxis.x:F1}, {unityAxis.y:F1}, {unityAxis.z:F1}) Angle({unityAngle:F1}°)");
        ...
        private bool ProcessTransform(string line, RWXParseContext context)
        {
            ...
            if (isInPrototype && isBedRelated)
            {
                ...
                string hierarchyPath = GetHierarchyPath(context);
                LogDebug($"🛏️ BED MATRIX | {hierarchyPath}");
                LogDebug($"   RWX: [{values[0]:F3}, {values[1]:F3}, {values[2]:F3}, {values[3]:F3}, {values[4]:F3}, {values[5]:F3}, {values[6]:F3}, {values[7]:F3}, {values[8]:F3}, {values[9]:F3}, {values[10]:F3}, {values[11]:F3}, {values[12]:F3}, {values[13]:F3}, {values[14]:F3}, {values[15]:F3}]");
                LogDebug($"   Unity: Det={det:F3}, Trans=({translation.x:F3}, {translation.y:F3}, {translation.z:F3})");
            }
        ...
        private bool ProcessJointTransformBegin(string line, RWXParseContext context)
        {
            ...
            LogDebug($"� JOINT TRANSFORM BEGIN - Stack depth: {context.jointTransformStack.Count}");
            LogDebug($"   📋 Pushed current joint transform to stack");
        ...
        private bool ProcessJointTransformEnd(string line, RWXParseContext context)
        {
            ...
            if (context.jointTransformStack.Count > 0)
            {
                ...
                LogDebug($"� JOINT TRANSFORM END - Restored from stack, depth: {context.jointTransformStack.Count}");
            }
            else
            {
                context.currentJointTransform = Matrix4x4.identity;
                LogDebug($"🔗 JOINT TRANSFORM END - Reset to identity (stack empty)");
            }
            return true;
        }
        ...
        private bool ProcessIdentityJoint(string line, RWXParseContext context)
        {
            ...
            context.currentJointTransform = Matrix4x4.identity;
            LogDebug($"🔗 IDENTITY JOINT - Reset joint transform to identity");
            return true;
        }
        ...
        private bool ProcessRotateJointTM(string line, RWXParseContext context)
        {
            ...
            LogDebug($"🔗 ROTATE JOINT TM - Axis: ({x:F3}, {y:F3}, {z:F3}), Angle: {angle:F1}°");
